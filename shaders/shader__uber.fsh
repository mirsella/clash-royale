...
#ifdef LIGHTMAP
  #ifdef NORMAL	
	vec3 normalSample = texture2D(normalTex, v_texCoord0).xyz * 2.0 - vec3(1.0, 1.0, 1.0);
	vec3 normal = normalize(normalize(v_tangent) * normalSample.x + normalize(v_binormal) * normalSample.y + normalize(v_normal) * normalSample.z);
	vec2 texCoordLightmap = normal.xy * vec2(0.5, -0.5) + vec2(0.5, 0.5);
  #else
	vec2 texCoordLightmap = v_texCoordLightmap;
  #endif
#endif
#ifdef LIGHTMAP_DIFFUSE
	color.rgb *= texture2D(lightmapDiffuse, texCoordLightmap).rgb;
#endif
#ifdef COLORIZE_COLOR
	color *= u_colorize;
#endif
...
#ifdef LIGHTMAP_SPECULAR
  #ifdef SPECULAR_TEX
	color.rgb += texture2D(lightmapSpecular, texCoordLightmap).rgb * texture2D(specularTex, v_texCoord0).rgb;
  #else
    #ifdef SPECULAR_COLOR
	  color.rgb += texture2D(lightmapSpecular, texCoordLightmap).rgb * u_specular.rgb;
	#else
	  color.rgb += texture2D(lightmapSpecular, texCoordLightmap).rgb;
    #endif
  #endif
